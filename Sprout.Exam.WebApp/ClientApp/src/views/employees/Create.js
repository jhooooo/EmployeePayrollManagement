...
export class EmployeeCreate extends Component {
  ...
  handleValidation() {
      ...
      if (!this.state.typeId) {
          formIsValid = false;
          errors["typeId"] = "Employee Type is required.";
      }
      ...
      if (this.state.tin !== "undefined") {
          if (!this.state.tin.match(/^\d{9}$/)) {
              formIsValid = false;
              errors["tin"] = "Tin must be of 9 digit";
          }
      }

      if (this.state.fullName !== "undefined") {
...
<div className='form-group col-md-6'>
  <label htmlFor='inputTin4'>TIN: *</label>
    ...
    <span className="error">{this.state.errors["tin"]}</span>
</div>
<div className='form-group col-md-6'>
